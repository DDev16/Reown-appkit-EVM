...
export function useContentUpload() {
  ...
  const uploadThumbnail = async (
    ...
    const storageRef = ref(
      storage,
      `thumbnails/${contentType}/${Date.now()}_${thumbnailFile.name}`
    );
  ...
  const uploadContentFile = async (
    ...
    const storageRef = ref(
      storage,
      `${path}/${Date.now()}_${contentFile.name}`
    );
  ...
  const uploadCourseLessonFiles = async (
    ...
    for (const lessonFile of sortedLessonFiles) {
      try {
        ...
        const fileUrl = await uploadContentFile(
          lessonFile.file,
          "courses",
          path
        );
        ...
        console.error(
          `Error uploading lesson file: ${lessonFile.title}`,
          error
        );
        throw error;
      }
  ...
  const updateAllUserAnalytics = async (contentType: ContentType) => {
    try {
      console.log(
        `Starting analytics update for all users after ${contentType} upload`
      );
      ...
      console.log(
        `Updated analytics for ${updatePromises.length} users after ${contentType} upload`
      );
    } catch (error) {
  ...
  const saveContentToFirestore = async (
    ...
    if (contentType === "videos" || contentType === "courses") {
      console.log(
        `New ${contentType} uploaded, updating analytics for all users`
      );
